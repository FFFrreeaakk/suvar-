...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.JKOMD
  if(!erkekrol) return message.reply(`JKOMD Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.JKOMD)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.JKOMD)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Jandarma Komando",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Jandarma Komando Üye Kaydı Yapıldı!")
