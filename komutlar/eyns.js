...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.YNS
  if(!erkekrol) return message.reply(`YNS Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.YNS)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.YNS)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    .setAuthor("Yunus Şube Üye Kaydı Yapıldı!")
