...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.SancaktarYakınK
  if(!erkekrol) return message.reply(`Sancaktar Baba Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.SancaktarYakınK)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.SancaktarYakınK)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Yakın Koruma",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Yakın.K Üye Kaydı Yapıldı!")
