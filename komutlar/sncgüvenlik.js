...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.SancaktarGüvenlik
  if(!erkekrol) return message.reply(`Sancaktar Güvenlik Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.SancaktarGüvenlik)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.SancaktarGüvenlik)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    .setAuthor("Güvenlik Üye Kaydı Yapıldı!")
