...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.SancaktarBaba
  if(!erkekrol) return message.reply(`Sancaktar Baba Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.SancaktarBaba)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.SancaktarBaba)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    .setAuthor("Baba Üye Kaydı Yapıldı!")
