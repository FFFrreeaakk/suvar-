...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.PÖH
  if(!erkekrol) return message.reply(`PÖH Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.PÖH)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.PÖH)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Polis Özel Harekat",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Polis Özel Harekat Üye Kaydı Yapıldı!")
