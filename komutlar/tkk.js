...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.KaraKuvvetleri
  if(!erkekrol) return message.reply(`KK Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.KaraKuvvetleri)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.KaraKuvvetleri)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "KaraKuvvetleri",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Special Forces Üye Kaydı Yapıldı!")
