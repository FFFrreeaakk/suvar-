...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.JHDB
  if(!erkekrol) return message.reply(`JHDB Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.JHDB)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    setTimeout(function(){user.roles.add(jkood.JHDB)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Jandarma Havacılık Daire Başkanlığı",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Jandarma Havacılık Daire Başkanlığı Üye Kaydı Yapıldı!")
