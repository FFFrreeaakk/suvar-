...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.TRFK
  if(!erkekrol) return message.reply(`TRAFİK Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.TRFK)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    setTimeout(function(){user.roles.add(jkood.TRFK)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Trafik Şube",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Trafik Şube Üye Kaydı Yapıldı!")
