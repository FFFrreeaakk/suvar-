...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.JTRFK
  if(!erkekrol) return message.reply(`JTRFK Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.JTRFK)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.JTRFK)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Jandarma Trafik",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Jandarma Trafik Üye Kaydı Yapıldı!")
