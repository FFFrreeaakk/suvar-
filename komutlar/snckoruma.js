...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.SancaktarKoruma
  if(!erkekrol) return message.reply(`Sancaktar Koruma Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.SancaktarKoruma)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.SancaktarKoruma)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    .setAuthor("Koruma Üye Kaydı Yapıldı!")
