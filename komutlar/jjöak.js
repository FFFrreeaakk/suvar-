...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.JOAK
  if(!erkekrol) return message.reply(`JÖAK Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.JOAK)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.JOAK)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Jandarma Özel Asayiş Komutanlığı",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Jandarma Özel Asayiş Komutanlığı Üye Kaydı Yapıldı!")
