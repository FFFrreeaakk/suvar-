...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.JASYS
  if(!erkekrol) return message.reply(`JASYS Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.JASYS)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.JASYS)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Jandarma Asayiş Şube",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Jandarma Asayiş Şube Üye Kaydı Yapıldı!")
