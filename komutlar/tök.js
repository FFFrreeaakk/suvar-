...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.ÖzelKuvvetler
  if(!erkekrol) return message.reply(`ÖK Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.ÖzelKuvvetler)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    setTimeout(function(){user.roles.add(jkood.ÖzelKuvvetler)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Özel Kuvvetler",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Özel Kuvvetler Üye Kaydı Yapıldı!")
