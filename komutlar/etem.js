...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.TEM
  if(!erkekrol) return message.reply(`TEM Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.TEM)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    setTimeout(function(){user.roles.add(jkood.TEM)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Terörle Mücadele",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Terörle Mücadele Üye Kaydı Yapıldı!")
