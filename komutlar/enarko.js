...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.NARKO
  if(!erkekrol) return message.reply(`NARKO Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.NARKO)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.NARKO)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Narkotik Şube",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Narkotik Şube Üye Kaydı Yapıldı!")
