...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.ASYS
  if(!erkekrol) return message.reply(`ASYS Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.ASYS)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.ASYS)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Asayiş Şube",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Asayiş Şube Üye Kaydı Yapıldı!")
