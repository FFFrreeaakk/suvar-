...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.SancaktarBaşK
  if(!erkekrol) return message.reply(`Sancaktar Baba Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.SancaktarBaşK)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.SancaktarBaşK)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    .setAuthor("Baş Koruma Üye Kaydı Yapıldı!")
