...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.EmniyetGenelMüdürlüğü
  if(!erkekrol) return message.reply(`Üye Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.EmniyetGenelMüdürlüğü)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
      if(!nick) return message.channel.send("Lütfen Bir İsim Girin.")
    setTimeout(function(){user.roles.add(jkood.EmniyetGenelMüdürlüğü)},3000)
    setTimeout(function(){user.roles.remove(jkood.kayıtsızrol)},4000)
    user.setNickname(`[E.G.M]${nick}`)
