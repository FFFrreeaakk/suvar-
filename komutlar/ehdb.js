...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.HDB
  if(!erkekrol) return message.reply(`HDB Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.HDB)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.HDB)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Hava Dairesi Başkanlığı",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Hava Dairesi Başkanlığı Üye Kaydı Yapıldı!")
