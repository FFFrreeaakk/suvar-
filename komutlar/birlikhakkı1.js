...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.BirinciBirlikHakkı
  if(!erkekrol) return message.reply(`Birlik Hakkı Rolü Ayarlanmamış!`)
    ...
    setTimeout(function(){user.roles.add(jkood.BirinciBirlikHakkı)},3000)
    setTimeout(function(){user.roles.remove(jkood.kayıtsızrol)},3000)
