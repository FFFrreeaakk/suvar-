...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.DenizKuvvetleri
  if(!erkekrol) return message.reply(`DK Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.DenizKuvvetleri)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.DenizKuvvetleri)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "Deniz Kuvvetleri",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Deniz Kuvvetleri Üye Kaydı Yapıldı!")
