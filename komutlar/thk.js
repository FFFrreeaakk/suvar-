...
exports.run = async (client, message, args) => {
  ...
  const erkekrol = await jkood.HavaKuvvetleri
  if(!erkekrol) return message.reply(`HK Rolü Ayarlanmamış!`)
    ...
    if (user.roles.cache.has(jkood.HavaKuvvetleri)) return message.reply("Bu Kişi Zaten Kayıtlı!")
    const nick = args[1];
    const rutbe = args[2];
    setTimeout(function(){user.roles.add(jkood.HavaKuvvetleri)},3000)
  ...
  db.push("jkood."+message.guild.id+user.user.id,{
    ...
    cinsiyet: "HavaKuvvetleri",
    kaydeden: message.author.id
  })
    
      const embed = new Discord.MessageEmbed()
    .setAuthor("Hava Kuvvetleri Üye Kaydı Yapıldı!")
